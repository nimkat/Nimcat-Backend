...
class BaseOperatorPerm:
    ...
    def has_filter_permission(self, info: ResolveInfo):
        return self.op_func(
            self.op1.has_filter_permission(
                info), self.op2.has_filter_permission(info)
        )
...
class AllowShopManager(BasePermission):
    """
    Allow performing action only for Shop Manager.
    """

    @classmethod
    def has_permission(cls, info: ResolveInfo) -> bool:
        return info.context.user.groups.filter(name="shop_manager").exists()


class AllowWriter(BasePermission):
    """
    Allow performing action only for Shop Manager.
    """
    ...
    @classmethod
    def has_permission(cls, info: ResolveInfo) -> bool:
        return info.context.user.groups.filter(name="writer").exists()


class AllowAccomodationManager(BasePermission):
    """
    Allow performing action only for Accomodation Manager.
    """

    @classmethod
    def has_permission(cls, info: ResolveInfo) -> bool:
        return info.context.user.groups.filter(name="accomodation_manager").exists()


class AllowTourLeader(BasePermission):
    """
    Allow performing action only for TourLeader.
    """

    @classmethod
    def has_permission(cls, info: ResolveInfo) -> bool:
        return info.context.user.groups.filter(name="tour_leader").exists()
